        ...
        stage('Docker Build') {
            steps {
                script {
                    ...
                    dockerImage = docker.build("mapservice-app:${env.BUILD_ID}")
                }
            }
        }
        ...
        stage('Push') {
            steps {
                script {
                    ...
                    sh "docker tag mapservice-app:${env.BUILD_ID} karimelhou/mapservice:${env.BUILD_ID}"
                    // Pushing the image to Docker Hub
                    sh "docker push karimelhou/mapservice:${env.BUILD_ID}"
                }
            }
        }
        ...
        stage('Docker Run') {
            steps {
                script {
                    ...
                    sh "docker run -d -p 8384:8384 mapservice-app:${env.BUILD_ID}"
                }
            }
        }
